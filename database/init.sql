...
-- Database initialization script for Fuel Efficiency Predictor (SQLite version)
...
CREATE TABLE IF NOT EXISTS prediction_history (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id TEXT DEFAULT (lower(hex(randomblob(4))) || '-' || lower(hex(randomblob(2))) || '-' || '4' || substr(lower(hex(randomblob(2))), 2) || '-' || substr('89ab', abs(random()) % 4 + 1, 1) || substr(lower(hex(randomblob(2))), 2) || '-' || lower(hex(randomblob(6)))),
...
-- Environmental impact tracking
CREATE TABLE IF NOT EXISTS environmental_stats (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    date DATE DEFAULT (date('now')),
    total_predictions INTEGER DEFAULT 0,
    avg_predicted_mpg REAL,
    total_co2_calculated REAL,
    trees_equivalent INTEGER,
    fuel_savings_gallons REAL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(date)
);
...
CREATE TABLE IF NOT EXISTS model_performance (
    ...
    r2_score REAL,
    training_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_active INTEGER DEFAULT 0,  -- BOOLEAN as INTEGER (0/1)
    UNIQUE(model_name, version)
);

-- Insert sample vehicle data (SQLite-compatible)
INSERT OR IGNORE INTO vehicles (make, model, year, engine_size, cylinders, transmission, fuel_type, drivetrain, weight, city_mpg, highway_mpg, combined_mpg, co2_emissions) VALUES
-- Hybrid/Electric vehicles
('Toyota', 'Prius', 2023, 1.8, 4, 'CVT', 'Hybrid', 'FWD', 3142, 58.0, 53.0, 56.0, 96.0),
('Toyota', 'Prius Prime', 2023, 1.8, 4, 'CVT', 'Plug-in Hybrid', 'FWD', 3365, 127.0, 108.0, 114.0, 28.0),
('Tesla', 'Model 3', 2023, 0.0, 0, 'Single-Speed', 'Electric', 'RWD', 4034, 132.0, 126.0, 129.0, 0.0),
('Tesla', 'Model S', 2023, 0.0, 0, 'Single-Speed', 'Electric', 'AWD', 4561, 120.0, 115.0, 118.0, 0.0),
('Honda', 'Insight', 2022, 1.5, 4, 'CVT', 'Hybrid', 'FWD', 2987, 55.0, 49.0, 52.0, 110.0),

-- Efficient gasoline vehicles
('Honda', 'Civic', 2023, 2.0, 4, 'CVT', 'Gasoline', 'FWD', 2906, 32.0, 42.0, 36.0, 262.0),
('Toyota', 'Corolla', 2023, 2.0, 4, 'CVT', 'Gasoline', 'FWD', 2955, 32.0, 41.0, 36.0, 258.0),
('Nissan', 'Versa', 2023, 1.6, 4, 'CVT', 'Gasoline', 'FWD', 2599, 32.0, 40.0, 35.0, 267.0),
('Mitsubishi', 'Mirage', 2023, 1.2, 3, 'CVT', 'Gasoline', 'FWD', 2018, 36.0, 43.0, 39.0, 238.0),

-- Mid-range vehicles
('Honda', 'Accord', 2023, 1.5, 4, 'CVT', 'Gasoline', 'FWD', 3131, 32.0, 42.0, 36.0, 267.0),
('Toyota', 'Camry', 2023, 2.5, 4, '8-Speed Automatic', 'Gasoline', 'FWD', 3310, 28.0, 39.0, 32.0, 296.0),
('Mazda', 'Mazda3', 2023, 2.5, 4, '6-Speed Automatic', 'Gasoline', 'FWD', 3064, 26.0, 35.0, 30.0, 317.0),

-- SUVs
('Toyota', 'RAV4', 2023, 2.5, 4, '8-Speed Automatic', 'Gasoline', 'AWD', 3605, 27.0, 35.0, 30.0, 318.0),
('Honda', 'CR-V', 2023, 1.5, 4, 'CVT', 'Gasoline', 'AWD', 3433, 27.0, 32.0, 29.0, 328.0),
('Subaru', 'Forester', 2023, 2.5, 4, 'CVT', 'Gasoline', 'AWD', 3449, 26.0, 33.0, 29.0, 330.0),

-- Trucks
('Ford', 'F-150', 2023, 3.3, 6, '10-Speed Automatic', 'Gasoline', 'RWD', 4021, 19.0, 24.0, 21.0, 555.0),
('Chevrolet', 'Silverado 1500', 2023, 2.7, 4, '8-Speed Automatic', 'Gasoline', 'RWD', 4520, 20.0, 23.0, 21.0, 567.0),
('Ram', '1500', 2023, 3.6, 6, '8-Speed Automatic', 'Gasoline', 'RWD', 4798, 20.0, 25.0, 22.0, 533.0);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_vehicles_make_model ON vehicles(make, model);
CREATE INDEX IF NOT EXISTS idx_vehicles_year ON vehicles(year);
CREATE INDEX IF NOT EXISTS idx_vehicles_mpg ON vehicles(combined_mpg);
CREATE INDEX IF NOT EXISTS idx_prediction_history_created_at ON prediction_history(created_at);
CREATE INDEX IF NOT EXISTS idx_prediction_history_mpg ON prediction_history(predicted_mpg);
CREATE INDEX IF NOT EXISTS idx_environmental_stats_date ON environmental_stats(date);

-- Create a view for dashboard statistics (SQLite supports VIEW)
...
-- Create a view for recent high-efficiency predictions
CREATE VIEW IF NOT EXISTS efficient_predictions AS
SELECT 
    json_extract(user_input, '$.make') as make,
    json_extract(user_input, '$.model') as model,
    predicted_mpg,
    green_score,
    created_at
FROM prediction_history 
...
-- Insert initial environmental stats
INSERT OR IGNORE INTO environmental_stats (date, total_predictions, avg_predicted_mpg, total_co2_calculated, trees_equivalent)
