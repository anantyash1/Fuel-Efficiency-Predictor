...
# Platform-specific imports (non-blocking)
try:
    import fcntl
    HAS_FCNTL = True
except ImportError:
...
# Force SQLite configuration (no PostgreSQL fallback)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///fuel_efficiency.db'
...
# Vehicle Model (your four factors: make, model, year, engine_size + essentials)
class Vehicle(db.Model):
    __tablename__ = 'vehicle'
...
def initialize_database():
    """Initialize database tables and check data (safe version)"""
    try:
        with app.app_context():
            # Create all tables
            db.create_all()
