...
class KaggleCSVProcessor:
    ...
    def analyze_dataset(self):
        ...
        if 'Year_from' in self.df.columns:
            year_range = f"{self.df['Year_from'].min()} - {self.df['Year_from'].max()}"
            print(f"\n📅 Year range: {year_range}")
        
        if 'capacity_cm3' in self.df.columns:
            engines = self.df['capacity_cm3'].dropna()
            if len(engines) > 0:
                print(f"\n🔧 Engine sizes (cm³): {engines.min()} - {engines.max()}")
    ...
    def clean_and_process_data(self):
        ...
        if 'Year_from' in processed_df.columns:
            ...
            print(f"  Remaining rows after year filter: {len(processed_df):,}")
